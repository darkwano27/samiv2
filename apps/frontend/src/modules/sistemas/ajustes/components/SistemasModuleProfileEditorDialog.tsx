/**
 * Modal crear/editar perfil Sistemas: acciones por app (read/create/update/delete).
 */
...
import {
  ...
  sisCreateModuleProfile,
  sisDeleteModuleProfile,
  sisFetchModuleProfileDetail,
  sisFetchProfileActionCatalog,
  sisUpdateModuleProfile,
...
export function SistemasModuleProfileEditorDialog({ open, mode, profileId, onClose }: Props) {
  ...
  const catalogQ = useQuery({
    queryKey: ['sistemas', 'module-settings', 'profile-action-catalog'],
    queryFn: sisFetchProfileActionCatalog,
    enabled: open,
  });

  const detailQ = useQuery({
    queryKey: ['sistemas', 'module-settings', 'module-profile', profileId ?? ''],
    queryFn: () => sisFetchModuleProfileDetail(profileId!),
  ...
  const createM = useMutation({
    mutationFn: sisCreateModuleProfile,
    onSuccess: async () => {
      setFormError(null);
    ...
    onError: async (e) => {
      setFormError((await readSisModuleSettingsMessage(e)) ?? 'No se pudo crear el perfil.');
    },
  });
  ...
  const updateM = useMutation({
    mutationFn: ({
      ...
      id: string;
      body: Parameters<typeof sisUpdateModuleProfile>[1];
    }) => sisUpdateModuleProfile(id, body),
    onSuccess: async () => {
      setFormError(null);
      await invalidateSisModuleSettingsRelatedQueries(qc);
    ...
    onError: async (e) => {
      setFormError((await readSisModuleSettingsMessage(e)) ?? 'No se pudo guardar el perfil.');
    },
  });

  const deleteM = useMutation({
    mutationFn: () => sisDeleteModuleProfile(profileId!),
    onSuccess: async () => {
      setFormError(null);
      await invalidateSisModuleSettingsRelatedQueries(qc);
      onClose();
    ...
    onError: async (e) => {
      setFormError((await readSisModuleSettingsMessage(e)) ?? 'No se pudo eliminar el perfil.');
    },
  });
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="sis-profile-editor-title"
        className={cn(
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3">
          <h2 id="sis-profile-editor-title" className="font-heading text-lg font-semibold">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="sis-prof-label">Nombre</Label>
                <Input
                  id="sis-prof-label"
                  value={label}
              ...
              <div className="space-y-2">
                <Label htmlFor="sis-prof-desc">Descripción (opcional)</Label>
                <textarea
                  id="sis-prof-desc"
                  value={description}
