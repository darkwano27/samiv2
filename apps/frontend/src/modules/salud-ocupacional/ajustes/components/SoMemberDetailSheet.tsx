/**
 * Panel lateral — detalle de miembro del módulo SO (Fase A roadmap).
 */
...
import {
  invalidateSoMemberDetailAndRelated,
  readSoModuleSettingsMessage,
  soFetchMemberDetail,
  soFetchModuleProfiles,
  soReplaceMemberProfile,
  soRevokeAllInModule,
  soRevokeAssignmentInModule,
  type SoMemberDetail,
...
export function SoMemberDetailSheet({ open, workerId, onClose }: Props) {
  ...
  const detailQ = useQuery({
    ...
    queryFn: () => soFetchMemberDetail(workerId!),
  ...
  const invalidateMembers = async () => {
    await invalidateSoMemberDetailAndRelated(qc, workerId);
  };

  const revokeOneM = useMutation({
    mutationFn: (assignmentId: string) => soRevokeAssignmentInModule(assignmentId),
    onSuccess: async () => {
      setFormError(null);
    ...
    onError: async (e) => {
      setFormError(
        (await readSoModuleSettingsMessage(e)) ?? 'No se pudo quitar la asignación.',
      );
    },
  });
  ...
  const replaceM = useMutation({
    mutationFn: () =>
      soReplaceMemberProfile(workerId!, profileSlug),
    onSuccess: async () => {
      setFormError(null);
      await invalidateMembers();
    ...
    onError: async (e) => {
      setFormError(
        (await readSoModuleSettingsMessage(e)) ?? 'No se pudo cambiar el perfil.',
      );
    },
  });

  const revokeAllM = useMutation({
    mutationFn: () => soRevokeAllInModule(workerId!),
    onSuccess: async () => {
      setFormError(null);
      onClose();
      await invalidateMembers();
    ...
    onError: async (e) => {
      setFormError(
        (await readSoModuleSettingsMessage(e)) ??
  ...
  function confirmReplace() {
    if (
      !window.confirm(
        'Se quitarán todos los permisos actuales de Salud ocupacional y se aplicará el perfil seleccionado. ¿Continuar?',
      )
    ) {
      return;
    }
  ...
  function confirmRevoke() {
    if (
      !window.confirm(
        '¿Quitar todo acceso a Salud ocupacional para este trabajador? No podrá usar ninguna app del módulo.',
      )
    ) {
      return;
    }
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="so-member-sheet-title"
        className={cn(
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <h2 id="so-member-sheet-title" className="font-heading text-lg font-semibold">
            Acceso en Salud ocupacional
          </h2>
          <Button
            type="button"
            variant="ghost"
