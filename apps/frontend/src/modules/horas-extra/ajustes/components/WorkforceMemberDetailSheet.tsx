/**
 * Panel lateral — detalle de miembro del módulo WorkForce.
 */
...
import {
  invalidateWfMemberDetailAndRelated,
...
export function WorkforceMemberDetailSheet({ open, workerId, onClose }: Props) {
  ...
  const detailQ = useQuery({
    ...
    queryFn: () => wfFetchMemberDetail(workerId!),
  ...
  const invalidateMembers = async () => {
    await invalidateWfMemberDetailAndRelated(qc, workerId);
  };

  const revokeOneM = useMutation({
    mutationFn: (assignmentId: string) => wfRevokeAssignmentInModule(assignmentId),
    onSuccess: async () => {
      setFormError(null);
    ...
    onError: async (e) => {
      setFormError(
        (await readWfModuleSettingsMessage(e)) ?? 'No se pudo quitar la asignación.',
      );
    },
  });

  const replaceM = useMutation({
    mutationFn: () => wfReplaceMemberProfile(workerId!, profileSlug),
    onSuccess: async () => {
      setFormError(null);
      await invalidateMembers();
    ...
    onError: async (e) => {
      setFormError(
        (await readWfModuleSettingsMessage(e)) ?? 'No se pudo cambiar el perfil.',
      );
    },
  });

  const revokeAllM = useMutation({
    mutationFn: () => wfRevokeAllInModule(workerId!),
    onSuccess: async () => {
      setFormError(null);
      onClose();
      await invalidateMembers();
    ...
    onError: async (e) => {
      setFormError(
        (await readWfModuleSettingsMessage(e)) ?? 'No se pudo revocar el acceso.',
      );
    },
  });
  ...
  function confirmReplace() {
    if (
      !window.confirm(
        'Se quitarán todos los permisos actuales del módulo WorkForce y se aplicará el perfil seleccionado. ¿Continuar?',
      )
    ) {
      return;
    }
  ...
  function confirmRevoke() {
    if (
      !window.confirm(
        '¿Quitar todo acceso al módulo WorkForce para este trabajador? No podrá usar ninguna app del módulo.',
      )
    ) {
      return;
    }
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="wf-member-sheet-title"
        className={cn(
      ...
      >
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <h2 id="wf-member-sheet-title" className="font-heading text-lg font-semibold">
            Acceso en WorkForce
          </h2>
          <Button
            type="button"
