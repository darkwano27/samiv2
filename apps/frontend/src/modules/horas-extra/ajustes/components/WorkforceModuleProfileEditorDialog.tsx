/**
 * Modal crear/editar perfil WorkForce (acciones por app).
 */
...
import {
  ...
  wfCreateModuleProfile,
  wfDeleteModuleProfile,
  wfFetchModuleProfileDetail,
  wfFetchProfileActionCatalog,
  wfUpdateModuleProfile,
...
export function WorkforceModuleProfileEditorDialog({ open, mode, profileId, onClose }: Props) {
  ...
  const catalogQ = useQuery({
    queryKey: ['workforce', 'module-settings', 'profile-action-catalog'],
    queryFn: wfFetchProfileActionCatalog,
    enabled: open,
  });

  const detailQ = useQuery({
    queryKey: ['workforce', 'module-settings', 'module-profile', profileId ?? ''],
    queryFn: () => wfFetchModuleProfileDetail(profileId!),
  ...
  const createM = useMutation({
    mutationFn: wfCreateModuleProfile,
    onSuccess: async () => {
      setFormError(null);
    ...
    onError: async (e) => {
      setFormError((await readWfModuleSettingsMessage(e)) ?? 'No se pudo crear el perfil.');
    },
  });
  ...
  const updateM = useMutation({
    mutationFn: ({
      ...
      id: string;
      body: Parameters<typeof wfUpdateModuleProfile>[1];
    }) => wfUpdateModuleProfile(id, body),
    onSuccess: async () => {
      setFormError(null);
      await invalidateWfModuleSettingsRelatedQueries(qc);
    ...
    onError: async (e) => {
      setFormError((await readWfModuleSettingsMessage(e)) ?? 'No se pudo guardar el perfil.');
    },
  });

  const deleteM = useMutation({
    mutationFn: () => wfDeleteModuleProfile(profileId!),
    onSuccess: async () => {
      setFormError(null);
      await invalidateWfModuleSettingsRelatedQueries(qc);
      onClose();
  ...
  return (
    ...
    >
      <div
        role="dialog"
        aria-modal="true"
        aria-labelledby="wf-profile-editor-title"
        className={cn(
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-border px-4 py-3">
          <h2 id="wf-profile-editor-title" className="font-heading text-lg font-semibold">
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {isSeed ? (
                ...
                <p className="rounded-md border border-border bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
                  Marcá acciones por aplicación. Si cambiás la plantilla, quien ya tenía el perfil no
                  cambia solo: hay que volver a aplicar el perfil a esa persona.
                </p>
              )}
              ...
              <div className="space-y-2">
                <Label htmlFor="wf-prof-label">Nombre</Label>
                <Input
                  id="wf-prof-label"
                  value={label}
              ...
              <div className="space-y-2">
                <Label htmlFor="wf-prof-desc">Descripción (opcional)</Label>
                <textarea
                  id="wf-prof-desc"
                  value={description}
